    ...
    <div className="flex flex-col gap-5 w-full">
      <div className='max-w-[600px] md:max-w-[900px] items-center lg:max-w-[1080px] p-2 text-[10px] md:text-sm lg:text-base flex gap-4 md:gap-5 lg:gap-6 mx-auto h-10'>
        <div
          ...
          className={props.activeCategory === "All" ? ' border bg-gray-300 bg-opacity-10 border-slate-600 px-2 py-1  rounded-lg ' : ''}
        >
          All
        </div>
        {categories.map((category) => (
          <div
            ...
            className={props.activeCategory === category._id ? ' border bg-gray-300 bg-opacity-10 border-slate-600 px-2 py-1  rounded-lg ' : ''}
          >
            {category.name}
          </div>
        ))}
