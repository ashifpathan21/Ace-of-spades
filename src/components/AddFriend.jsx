...
const AddFriend = (props) => {
  ...
  async function send() {
    ...
    try {
      const responce = await dispatch(
        sendRequest({ friendId: friend._id }, token)
      );
      if (responce) {
        setSent(true);
  ...
  return (
    <div
      className={`w-full px-4  items-center backdrop-blur-3xl shadow-cyan-300 shadow-sm hover:shadow-md transition-all duration-500 rounded-lg p-3 justify-around flex`}
    >
      <img
        onClick={() => {
      ...
      >
        <div className="text-center flex flex-col gap-1 sm:text-left">
          <h2 className="text-lg  sm:text-xl capitalize lg:text-2xl">
          ...
          !isAdded && (
            ...
            >
              <button
                ...
                disabled={sent}
                className="bg-[#4bffe4] rounded-xl  px-3  inline  py-2  text-gray-600 lg:font-semibold md:font-semibold hover:bg-[#a2f5e9] "
              >
                {sent ? (
                  "Request Sent"
                ) : (
                  <>
                    <i className="ri-user-add-fill"></i>Add Friend{" "}
                  </>
                )}
              </button>
            </Tilt>
