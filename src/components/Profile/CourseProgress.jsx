...
setTotalSubsections(subsections)
  ...
  return (
    ...
    transition-all duration-300 hover:shadow-xl hover:border-gray-300">
      {/* course name */}
      <h2 className='w-full p-2 text-left text-md font-medium'>{course?.courseName}</h2>
      ...
      <input 
        ...
        style={{
          background: `linear-gradient(90deg, orange ${(completedSubSections / totalSubsections) * 100}%, white 1%)`
        }} 
      />

      <p className='text-gray-500 text-sm'>{completedSubSections + '/' + totalSubsections}</p>
    </div>
  )
}
