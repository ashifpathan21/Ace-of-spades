...
const EditProfile = (props) => {
  ...
  useEffect(() => {
    ...
    if (college == additionalDetails.collegeName) return;
    if (debounceTimer) clearTimeout(debounceTimer);

    const timer = setTimeout(() => {
      dispatch(fetchColleges(college));
    }, 300);
  ...
  async function update(e) {
    ...
    try {
      const payload = await dispatch(
        updateProfile(payload1, token, setLoading)
      );
  ...
  return (
    <div className="min-h-screen h-full">
      <div
        onClick={() => back()}
        className=" min-h-full absolute z-10 right-10 top-3 text-2xl rounded-lg"
      >
      ...
      {loading ? (
        ...
        <form
          onSubmit={update}
          className=" p-5 md:p-8 lg:p-10 max-w-[800px] mx-auto mb-10  w-full relative min-h-screen "
        >
          {/* Profile image */}
          <div className="w-full mb-3 flex justify-center  items-center">
            <div className="rounded-full relative flex justify-center items-center backdrop-blur-3xl shadow shadow-cyan-300 h-30 w-30">
              <img
                src={img}
                alt="Profile"
                className="rounded-full object-cover aspect-square h-29 w-29"
              />
              <i
